...
with st.container():
    st.markdown("<p style='text-align: center; font-weight: bold;'>Quantidade de restaurantes  registrados por país</p>", unsafe_allow_html=True)
    
    fig = fp.create_graph(df1, 'restaurant_id', 'country', 'count', 'Quantidade de restaurantes', 'País')
    st.plotly_chart(fig, use_container_width=True)

with st.container():
    st.markdown("<p style='text-align: center; font-weight: bold;'>Quantidade de cidades  registrados por país</p>", unsafe_allow_html=True)
...
with st.container():
    ...
    with col1:
        col1.markdown("<p style='text-align: center; font-weight: bold;'>Média de avaliações feitas por país</p>", unsafe_allow_html=True)

        fig = fp.create_graph(df1, 'votes', 'country', 'mean', 'Média de avaliações', 'País')
        st.plotly_chart(fig, use_container_width=True)

    with col2:
        col2.markdown("<p style='text-align: center; font-weight: bold;'>Média preço de um prato para duas pessoas por país </p>", unsafe_allow_html=True)
